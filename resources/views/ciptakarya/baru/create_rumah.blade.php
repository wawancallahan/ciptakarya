        ...
        $(function () {
            $('#btn-proses').on('click', function (e) {
                ...
                if (form[0].checkValidity()) {
                    ...
                    $.ajax({
                        method: 'POST',
                        url: '{{ route("api.ciptakarya.rumah") }}',
...
@section ('content')
    ...
    <form action="{{ route('ciptakaryabaru.store') }}" method="POST" id="form-cipta" enctype="multipart/form-data">
        ...
        <div class="card card-primary">
            <div class="card-header">
                <h3 class="card-title">Rumah Negara</h3>
            </div>
            <div class="card-body">
                <input type="hidden" name="jenis" value="rumah">

                <div class="form-group">
                    <label for="">Rumah Negara</label>
                    <select class="form-control" name="tipe" required>
                        @foreach ($rumahNegaras as $rumahNegaraId => $rumahNegaraValue)
                            <option value="{{ $rumahNegaraId }}">{{ $rumahNegaraValue }}</option>
                        @endforeach
                    </select>
                </div>
